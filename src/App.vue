<template>
  <v-app>
    ...
    <v-container class="mx-16 mt-16 pt-16" v-if="!running">
      <v-row>  
      <v-textarea
        variant="filled"
...
<script setup>
  ...
  async function submit_data (event) {
    ...
    const response = await register_data(data.value)
  ...
  async function register_data(data){
    const url = constants.BACKEND + "register_data"
    try {
      const response = await fetch(url, {
        method: 'POST',
        headers: {
